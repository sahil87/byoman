# Feature Specification: Byobu Session Manager

**Feature Branch**: `001-byobu-session-manager`
**Created**: 2026-01-30
**Status**: Draft
**Input**: User description: "CLI tool to manage byobu sessions with session listing, metrics display, and session actions"

## User Scenarios & Testing *(mandatory)*

### User Story 1 - View All Sessions (Priority: P1)

As a developer who uses multiple byobu sessions for different projects, I want to see a list of all my sessions so that I can quickly understand what's running and choose which session to work with.
...
**Independent Test**: Can be fully tested by running the tool and verifying it displays all existing byobu sessions with their metrics. Delivers immediate value by providing session awareness.

**Acceptance Scenarios**:

1. **Given** multiple byobu sessions exist, **When** I run the tool, **Then** I see a list of all sessions with their names
...
4. **Given** sessions exist, **When** I view the list, **Then** the information is current (sourced directly from byobu)

---

### User Story 2 - Attach to Existing Session (Priority: P1)

As a developer returning to work, I want to easily attach to an existing session so that I can resume my work without remembering byobu command syntax.
...
3. **Given** a session is already attached elsewhere, **When** I select it, **Then** I can still attach (byobu allows multiple attachments)

---

### User Story 3 - Start New Session (Priority: P2)

As a developer starting a new project, I want to create a new byobu session so that I can organize my work without leaving the interface.

**Why this priority**: Creating sessions is important but less frequent than viewing/attaching. Users can always create sessions via regular byobu commands if needed.
...
1. **Given** I am viewing the session list, **When** I choose to create a new session, **Then** a new byobu session is created
2. **Given** I create a new session, **When** I provide a name, **Then** the session is created with that name
3. **Given** I create a new session without a name, **When** the session is created, **Then** byobu assigns a default name
...
- What happens when byobu is not installed? Display a clear error message indicating the dependency is required.
- What happens when the user has no permissions to access byobu sockets? Display an appropriate error message.
...
- **FR-001**: System MUST display a list of all existing byobu sessions
...
- **FR-012**: System MUST source all session data directly from byobu (no config files or hidden state)
- **FR-013**: System MUST work on any system with byobu installed (no additional runtime dependencies; implemented in Go as single binary)
...
- **Session**: A byobu session with attributes: name (unique identifier), created time, last attached time, attachment status, window count
...
- **SC-003**: All displayed session information accurately reflects current byobu state (no stale data)
...
- Users have byobu installed on their system (which uses tmux as its backend)
- Users are comfortable with command-line interfaces
- The tool will use byobu commands to query session information
...
- **Dependency**: byobu (which uses tmux as its backend)
- **Terminology**: "byobu" is the canonical term; BYOMAN = BYObu MANager
...
- Q: What is the dependency? → A: byobu (which uses tmux as backend)
- Q: Should "byobu" or "tmux" be the canonical term? → A: "byobu" (BYOMAN = BYObu MANager)
